...
function createTemplateParser(gantt:Gantt) {
    return newRecadoInstance({
        templates:{
            [FIXED_ELEMENT_LABELS]:`<div class="vjs-gantt-task-labels" data-vjs-not-draggable="true">
                    <r-each in="entries" key="id">
                        <div data-vjs-type="{{type}}" class="vjs-gantt-task-label" style="height:${gantt.rowHeight}px;margin-left:{{indent}}rem">{{name}}</div>
                        </r-each>
                        </div>`,
            [FIXED_ELEMENT_TIMELINE]:`<div class="vjs-gantt-timeline" style="width:{{width}}px" data-vjs-not-draggable="true">
                    <r-each in="headers" key="id">
                        <div class="vjs-gantt-timeline-row vjs-gantt-timeline-{{id}}">
                            <r-each in="values" id="id">
                                <div class="vjs-gantt-timeline-entry" style="flex-basis:{{size}}px;height:${gantt.rowHeight}px">
                                    <r-if test="type == 'day'">
            ...
            [FIXED_ELEMENT_DAY_STRIPES]:`<div class="vjs-gantt-day-stripes">
                                            <r-each in="days" id="id">
                                                <div class="{{clazz}}" style="flex-basis:{{size}}px;height:{{height}}px"/>
                                            </r-each>
                                        </div>`,
            timelineLabel:(gantt.showDayName && gantt.showDayNumber) ? `<span>{{day}}</span><span class="vjs-gantt-day-name">{{label}}</span>` : gantt.showDayNumber ? `<span>{{day}}</span>` : `<span>{{label}}</span>`,
            [FIXED_ELEMENT_RIGHT_NOW]:`<div class="vjs-gantt-right-now" style="height:{{height}}px"/>`
        }
    })
}
...
export class GanttDecorator extends Decorator {
    ...
    private _decorateBody(params:DecorateParams) {
        ...
        const stripeHeight = params.model.getNodes().length * this.gantt.rowHeight
        const dayRange = millisecondsToDays(this.gantt.maxValue.current - this.gantt.minValue.current)
    ...
    reset(params:DecorateResetParams) {
        //this.labels && params.remove(this.labels)
        // this.timelines && params.remove(this.timelines)
    }
