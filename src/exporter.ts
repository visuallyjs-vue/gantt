import {Edge, VisuallyJsModel} from "@visuallyjs/browser-ui"
import {Gantt,GanttExporterParameters, SerializedTask, TaskEntry} from "./defs"
 ...
 * @param entry
 */
function getDependencies(model:VisuallyJsModel, entry:TaskEntry):Array<string> {

    return model.getAllEdgesFor(entry.node, (e:Edge) => e.target === entry.node).map(e => e.source.id)
}
